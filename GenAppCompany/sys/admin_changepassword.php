...
<!-- BEGIN MAIN CONTENT -->
                                                ...
                                                <div class="col-sm-12">
                                                    <div class="align-center">
                                                        <button type="submit" class="btn btn-primary m-r-20">เปลี่ยนรหัสผ่าน</button>
                                                          <a href="admin_index.php?page=admindashboard"><button type="reset" class="btn btn-default" onclick="return  confirm('คุณต้องการยกเลิก ใช่หรือไม่')">ยกเลิก </button></a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
          ...
          if($newpass != $confirmNewpass){
            echo '<script type="text/javascript"> window.alert("รหัสผ่านไม่ตรงกัน !"); window.location.href = "admin_index.php?page=adminchangepassword";</script>';
            exit();
        }
        else{
            $isUpdatePasssword = $functionClass->updatePassword($newpass);
            if($isUpdatePasssword){
              echo '<script type="text/javascript"> window.alert("เปลี่ยนรหัสผ่านสำเร็จ !"); window.location.href = "admin_index.php?page=admindashboard";</script>';
            }else{
              echo '<script type="text/javascript"> window.alert("ไม่สามารถเปลี่ยนรหัสผ่านได้ !"); window.location.href = "admin_index.php?page=adminchangepassword";</script>';
            }
        }
     }
