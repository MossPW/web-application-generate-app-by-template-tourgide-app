...
//more pic location
//1
function showMoreLocationPic1(input) {
$('#showmorepiclocation1').show();
    if (input.files && input.files[0]) {
        var reader = new FileReader();

        reader.onload = function (e) {
            $('#showmorepiclocation1')
                .attr('src', e.target.result)
                .width(200)
                .height(200);
        };
...
//2
function showMoreLocationPic2(input) {
$('#showmorepiclocation2').show();
    if (input.files && input.files[0]) {
        var reader = new FileReader();

        reader.onload = function (e) {
            $('#showmorepiclocation2')
                .attr('src', e.target.result)
                .width(200)
                .height(200);
        };
...
//3
function showMoreLocationPic3(input) {
$('#showmorepiclocation3').show();
    if (input.files && input.files[0]) {
        var reader = new FileReader();

        reader.onload = function (e) {
            $('#showmorepiclocation3')
                .attr('src', e.target.result)
                .width(200)
                .height(200);
        };
...
switch (page) {
  ...
  var app_id = getDataUrl('app_id');
   if(app_id==null){
    ...
    $('#reset_appcreate').click(function() {
  ...
  $('#form_editapp').submit(function(e) {
    ...
    var msg=set_data_file(form_data);
    //alert(msg);
    if(msg=="same app_name_en"){ //insert fail
      alert("ชื่อแอปพลิเคชั่นภาษาอังกฤษถูกใช้ไปแล้ว กรุณาระบุใหม!");
    }
    else if(msg=="1"){
      //alert(msg);
...
case  'mainlocationcreate' :////**case page****/////
  ...
  $('#form_addmainlocation').submit(function(a) {
    ...
    //alert(msg);
    window.location = "index.php?page=editapp&app_id=" + getDataUrl('app_id');
          a.preventDefault();
          a.unbind();
    });
  ...
  case  'locationcreate' :////**case page****/////
   ...
   if (isLocation_app_id==null) {
    ...
    $('#form_addlocation').submit(function(a) {
      ...
      var location_more_pic = $('#location_more_pic')[0].files;
      var form_data2 = new FormData();
      ...
      if(location_more_pic.length>0){
      for(var i=0; i<location_more_pic.length; i++)
        {
        form_data2.append("location_more_pic[]", document.getElementById('location_more_pic').files[i]);
        }
      }
    else{
      form_data2.append("location_more_pic[]", document.getElementById('location_more_pic').files[0]);
    }
            var msg=/*"5555";*/set_data_file(form_data2);
          //  alert(msg);
    ...
    $('#addquestion').click(function() {
     ...
     $('#success_location').click(function() {
      ...
      $('#reset_locationcreate').click(function() {
             window.location = "index.php?page=editapp&app_id=" + getDataUrl('app_id');
       });
  break;
  ...
  last_app_id = getDataUrl('app_id');
     $('#form_addquestion').submit(function(a) {
        ...
        if($(".type1").is(":visible")){
          ...
          var choice1 =   $('input[name=choice1_type1]').val();
          var choice2 =   $('input[name=choice2_type1]').val();
          var choice3 =   $('input[name=choice3_type1]').val();
          var choice4 =   $('input[name=choice4_type1]').val();

        if(answer_text!=""&&choice1!=""&&choice2!=""&&choice3!=""&&choice4!=""){
            var form_q = {
        ...
        //type 2 visible
         ...
         var choice1 =   $('input[name=choice1_type2]').val();
         var choice2 =   $('input[name=choice2_type2]').val();

       if(answer_text!=""&&choice1!=""&&choice2!=""){
         var form_q = {
           'case'  : "form_createquestion",
           'location_app_id' : location_app_id,
           'type_question' : "2",
       ...
       var choice1 =   $('input[name=choice1_type3]').val();
       var choice2 =   $('input[name=choice2_type3]').val();
       var choice3 =   $('input[name=choice3_type3]').val();
       var choice4 =   $('input[name=choice4_type3]').val();
     ...
     if(answer_text!=""&&choice1!=""&&choice2!=""&&choice3!=""&&choice4!=""){
      ...
      //  alert("type4 show");
        ...
        var choice_str = $('input[name=choice1_type4]').val()+"_"+$('input[name=choice2_type4]').val()+"_"+$('input[name=choice3_type4]').val()+"_"+$('input[name=choice4_type4]').val();
               ...
               var choice1 =   $('input[name=choice1_type4]').val();
               var choice2 =   $('input[name=choice2_type4]').val();
               var choice3 =   $('input[name=choice3_type4]').val();
               var choice4 =   $('input[name=choice4_type4]').val();
   ...
   if(answer_text!=""&&choice1!=""&&choice2!=""&&choice3!=""&&choice4!=""){
      ...
      if(msg!="Fill in"){
      window.location = "index.php?page=editlocation&app_id=" + last_app_id+"&location_app_id="+location_app_id;
    }
    else{
    ...
    $('#reset_question').click(function() {
          window.location = "index.php?page=editlocation&app_id=" + last_app_id+"&location_app_id="+location_app_id;
     });
break;
   case  'editquestion' :
   var location_app_id = getDataUrl('location_app_id');
   var question_app_id = getDataUrl('question_app_id');
   var questionType = getDataUrl('questiontype');
   last_app_id = getDataUrl('app_id');
   var msg="";
      $('#form_addquestion').submit(function(a) {
        ...
        //   alert("type1 only");
             ...
             var form_q = {
               ...
               'choice_str' : choice_str,
               'answer_text' : answer_text,
               'question_app_id' : question_app_id
             };
           msg=set_data(form_q);
        //  alert(msg);
         }
...
//change inputfile
$('#location_more_pic').change(function(){
  //$('#location_more_pic').val('');
 var error_images = '';
   $('#error_location_more_pic').html("<span class='text-danger'>"+error_images+"</span>");
 var files = $('#location_more_pic')[0].files;
 if(files.length > 3)
 {
  error_images += 'คุณสามารถเพิ่มรูปภาพได้ไม่เกิน 3 รูป';
 }
 else
 {
  for(var i=0; i<files.length; i++)
  {
   var name = document.getElementById("location_more_pic").files[i].name;
   ...
   oFReader.readAsDataURL(document.getElementById("location_more_pic").files[i]);
   var f = document.getElementById("location_more_pic").files[i];
...
//
//show image multiple file location
   ...
   if (input.files) {
       ...
       for (i = 0; i < filesAmount; i++) {
           ...
           reader.onload = function(event) {
               $($.parseHTML('<img>')).attr('src', event.target.result).addClass( "moreLocationImagesPreview" ).appendTo(placeToInsertImagePreview).width(200)
           .height(200);
           }
...
$('#location_more_pic').on('change', function() {
   $('.moreLocationImagesPreview').remove();
